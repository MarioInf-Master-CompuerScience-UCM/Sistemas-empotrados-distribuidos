...
void Led_Display(int LedStatus){
	...
	if ((LedStatus & 0x01) == 0x01){
		rPDATB=rPDATB & ~(0x01<<9);		// poner a 0 el bit 9 del registro de datos del puerto B
	}else{
		rPDATB=rPDATB |(0x01<<9);		// poner a 1 el bit 9 del registro de datos del puerto B
	}

	if ((LedStatus & 0x02) == 0x02){
		rPDATB=rPDATB & ~(0x01<<10);	// poner a 0 el bit 10 del registro de datos del puerto B
	}else{
		rPDATB=rPDATB |(0x01<<10);		// poner a 1 el bit 10 del registro de datos del puerto B
	}
